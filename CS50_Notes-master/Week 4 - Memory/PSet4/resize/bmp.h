...
{
    WORD bfType;
    DWORD bfSize; //size increases if n > 1
    WORD bfReserved1;
    WORD bfReserved2;
    DWORD bfOffBits;
...
typedef struct
{
    DWORD biSize; //size increases if n > 1
    LONG biWidth; //size increases if n > 1
    LONG biHeight; //size increases if n > 1
    WORD biPlanes;
    WORD biBitCount;
    DWORD biCompression;
    DWORD biSizeImage; //size increases if n > 1
    LONG biXPelsPerMeter; //size increases if n > 1
    LONG biYPelsPerMeter; //size increases if n > 1
    DWORD biClrUsed;
